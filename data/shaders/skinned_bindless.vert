...
layout(set = 1, binding = 4) readonly buffer SkinnedMeshDrawCommands {
    uint culled_count;
    IndexedDrawCommand cmds[];
};
...
layout(scalar, set = 3, binding = 0) readonly buffer SkinnedVertexBuffer {
...
void main() {
    ...
    uint base = gl_VertexIndex + draw.animation_output_offset;
    Vertex vertex = skinned_vertices[base];
