...
RenderPass& Framegraph::add_pass(const std::string& name) {
    return passes.emplace_back(RenderPass{.name = name});
}

void Framegraph::import_image(const Image& image, VkImageLayout expected_layout, bool clear_on_first_write) {
    tracked_layouts.insert({image.handle, {expected_layout, clear_on_first_write}});
}

const PassTiming& Framegraph::get_pass_timing(const std::string& pass_name) {
...
void Framegraph::execute(VkCommandBuffer command_buffer, uint32_t frame_index) {
...
void Framegraph::gather_timestamp_queries(VkDevice device, float timestamp_period) {
...
float PassTiming::get_avg_timing_ms() const {
    double avg_time = 0.0;
...
void PassTiming::add_sample(double sample_ms) {
...
RenderPass& RenderPass::render_func(std::function<void(VkCommandBuffer command_buffer, uint32_t frame_index)> func) {
    on_render = func;

    return *this;
}

RenderPass& RenderPass::writes_buffer_dynamic(
...
RenderPass& RenderPass::reads_buffer_dynamic(
    const Buffer& buffer, VkPipelineStageFlagBits2 stage, VkAccessFlagBits2 access, VkDeviceSize size
) {
...
RenderPass& RenderPass::writes_buffer(
    const Buffer&            buffer,
...
RenderPass& RenderPass::reads_buffer(
    const Buffer&            buffer,
    VkPipelineStageFlagBits2 stage,
    VkAccessFlagBits2        access,
    VkDeviceSize             offset,
...
RenderPass& RenderPass::writes_storage_image(const Image& image, VkPipelineStageFlagBits2 stage) {
    return writes_image(image, stage, VK_ACCESS_2_SHADER_STORAGE_WRITE_BIT, VK_IMAGE_LAYOUT_GENERAL);
}

RenderPass& RenderPass::writes_depth_attachment(const Image& image) {
    return writes_image(
        image,
...
RenderPass& RenderPass::writes_color_attachment(const Image& image) {
    return writes_image(
        image,
...
RenderPass& RenderPass::writes_image(
    const Image&             image,
    VkPipelineStageFlagBits2 stage,
    VkAccessFlagBits2        access,
...
RenderPass& RenderPass::reads_storage_image(const Image& image, VkPipelineStageFlagBits2 stage) {
    return reads_image(image, stage, VK_ACCESS_2_SHADER_STORAGE_READ_BIT, VK_IMAGE_LAYOUT_GENERAL);
}

RenderPass& RenderPass::samples_image(const Image& image, VkPipelineStageFlagBits2 stage) {
    return reads_image(image, stage, VK_ACCESS_2_SHADER_SAMPLED_READ_BIT, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
}

RenderPass& RenderPass::reads_image(
    const Image&             image,
    VkPipelineStageFlagBits2 stage,
    VkAccessFlagBits2        access,
    VkImageLayout            layout,
